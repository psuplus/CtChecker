...
void Infoflow::processGetElementPtrInstSource(const Value *source, std::set<const ConsElem *>& sourceSet, std::set<const AbstractLoc *> locs) {
  ...
  const Value * sourceptr = gep->getPointerOperand();

  if(const LoadInst *li = dyn_cast<LoadInst>(sourceptr)){
    const Value * load_target = li->getPointerOperand();
    errs() << "Load Target: "; load_target->dump(); errs() << "\n";
    sourceptr = load_target;
  }

  errs() << " SOURCE VALUE: "; sourceptr->dump(); errs() << "\n";
  std::set<const AbstractLoc *> sourceval = locsForValue(*sourceptr);

  errs() << "DUMPING ALL OF THE SOURCE OPERANDS\n";
  for(auto & sourcelocs: sourceval){
    sourcelocs->dump();
    //sourcelocs->dumpParentGraph();
  }
  ...
  std::set<const AbstractLoc *> constrainLocs;
  if(sourceval.size() > 0)
    constrainLocs = sourceval;
...
std::set<const ConsElem*> Infoflow::findRelevantConsElem(const AbstractLoc* node, std::map<unsigned, const ConsElem *> elemMap, unsigned offset){
  ...
  if (node->isNodeCompletelyFolded()){
    ...
    errs() << "FOUND ELEMENT: "; elemMap[offset]->dump(errs()); errs() << "\n";
...
Infoflow::getOrCreateConsElemTyped(const AbstractLoc &loc, unsigned numElements, const Value* v) {
  ...
  if (curElem == locConstraintMap.end() || (curElem != locConstraintMap.end() && curElem->second.size() == 1)) {
    ...
    for(auto & elem: curElem->second){
      elem.second->dump(errs());
    }
    return (curElem->second);
  }
}
...
Infoflow::createConsElemFromStruct(const AbstractLoc& loc , StructType * s) {
  errs() << " CREATED ELEMEMENT MAP FOR " << &loc << "\n";
  std::map<unsigned, const ConsElem *> elemMap;
  const DataLayout &TD = loc.getParentGraph()->getDataLayout();
...
Infoflow::putOrConstrainConsElem(bool implicit, bool sink, const AbstractLoc &loc, const ConsElem &lub, unsigned offset, unsigned numElements) {
  ...
  errs() << "putORConstrainConsElemNON: ";
  loc.dump();
  if(elemMap.size() == 0)
    return;
...
Infoflow::getOrCreateMemoryConsElem(const Value & value) {
    ...
    for (std::set<const AbstractLoc *>::iterator loc = locs.begin(), end = locs.end();
            loc != end ; ++loc) {
      std::map<unsigned, const ConsElem *> elemMap = getOrCreateConsElem(**loc);
      errs() << "MemConsElem Loc Size: " << (*loc)->getSize() <<"\n";
      if(hasOffset){
        const ConsElem * elem = elemMap[offset];
        if (join == NULL) {
          join = elem;
