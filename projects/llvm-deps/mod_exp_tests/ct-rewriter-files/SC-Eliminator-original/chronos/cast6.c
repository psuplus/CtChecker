...
inline int cast6_setkey(struct cast6_ctx *c, const uint8_t *in_key,
			unsigned key_len) __attribute__((always_inline))
{
	int i;
	uint32_t key[8];
	uint32_t p_key[8]; 
...
int main(int argc, char *argv[])
{
    ...
    {
    	key_len = 32;
    	printf("Warning: Only the first 32 bytes key is used!\n");
    }
 ...
 //    	printf("%02X",in[i]);
	...
	cast6_setkey(&ctx, in_key, key_len);
#ifdef gem5            
	m5_reset_stats(0, 0);
#endif
