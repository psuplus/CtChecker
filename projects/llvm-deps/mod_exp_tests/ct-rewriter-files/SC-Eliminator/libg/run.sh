...
if [[ "$NAME" == *"opt"* ]]; then
        if [[ "$NAME" == *"camellia"* ]]; then
                cp config-camellia-opt.json config.json
        elif [[ "$NAME" == *"des"* ]]; then
                cp config-des-opt.json config.json
        elif [[ "$NAME" == *"seed"* ]]; then
                cp config-seed-opt.json config.json
        elif [[ "$NAME" == *"twofish"* ]]; then
                cp config-twofish-opt.json config.json
        else
        ...
        if [[ "$NAME" == *"camellia"* ]]; then
                cp config-camellia-wo-opt.json config.json
        elif [[ "$NAME" == *"des"* ]]; then
                cp config-des-wo-opt.json config.json
        elif [[ "$NAME" == *"seed"* ]]; then
                cp config-seed-wo-opt.json config.json
        elif [[ "$NAME" == *"twofish"* ]]; then
                cp config-twofish-wo-opt.json config.json
        else
                echo "Wrong example name"
                exit 1
        fi
