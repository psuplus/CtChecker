...
if [[ "$NAME" == *"opt"* ]]; then
        if [[ "$NAME" == *"aes_core"* ]]; then
                cp config-aes_core-opt.json config.json
        elif [[ "$NAME" == *"cast-ssl"* ]]; then
                cp config-cast-ssl-opt.json config.json
        else
        ...
        if [[ "$NAME" == *"aes_core"* ]]; then
                cp config-aes_core-wo-opt.json config.json
        elif [[ "$NAME" == *"cast-ssl"* ]]; then
                cp config-cast-ssl-wo-opt.json config.json
        else
                echo "Wrong example name"
                exit 1
        fi
