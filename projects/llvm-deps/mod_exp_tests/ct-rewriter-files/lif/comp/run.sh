...
echo "Running $NAME with flags: $COL"

# change config file for each example
rm config.json
if [[ "$NAME" == *"opt"* ]]; then
        cp config-opt.json config.json
else
        cp config-wo-opt.json config.json
fi
...
RESULTPATH="${LEVEL}/projects/llvm-deps/mod_exp_tests/ct-rewriter-files/lif/results.csv"
