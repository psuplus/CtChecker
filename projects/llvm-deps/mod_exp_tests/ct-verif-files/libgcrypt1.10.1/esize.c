         ...
         struct karatsuba_ctx *karactx_p,int pub_branch0)
{
  if( ssize < KARATSUBA_THRESHOLD )
    dummy++; //_gcry_mpih_mul ( xp, rp, rsize, sp, ssize );
  else
    dummy++; //_gcry_mpih_mul_karatsuba_case (xp, rp, rsize, sp, ssize, karactx_p);

   if (pub_branch0) // if (rsize + ssize > msize)
    {
      dummy++; //_gcry_mpih_divrem (xp + msize, 0, xp, rsize + ssize, mp, msize);
      *xsize_p = msize;
    }
   else
     *xsize_p = rsize + ssize;
