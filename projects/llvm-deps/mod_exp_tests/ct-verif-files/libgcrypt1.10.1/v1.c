                ...
                gcry_mpi_t base, gcry_mpi_t expo, gcry_mpi_t mod, int pub_int0,int pub_int1,int pub_int2,mpi_limb_t *pub_l0,mpi_limb_t *pub_l1,mpi_limb_t *pub_l2,mpi_limb_t *pub_l3,mpi_limb_t *pub_l4,mpi_limb_t *pub_l5,mpi_limb_t *pub_l6,mpi_limb_t *pub_l7,mpi_limb_t pub_l8,int pub_branch0,int pub_branch1,int pub_branch2,int pub_branch3,int pub_branch4,int pub_branch5,int pub_branch6,int pub_branch7,int pub_branch8,int pub_branch9,int pub_branch10,int pub_branch11,int pub_branch12,int pub_branch13,int pub_branch14,int pub_branch15,int pub_branch16,int pub_branch17,int pub_branch18,int pub_branch19,int pub_branch20,int pub_branch21,int pub_branch22,int pub_branch23,int pub_branch24)
{
  ...
  /* Normalize MOD (i.e. make its most significant bit set) as
     ...
     obtained after a final reduction using the original MOD value. */
  mp_nlimbs = msec? msize:0;
  mp = mp_marker = pub_l0; //mpi_alloc_limb_space(msize, msec);
  count_leading_zeros (mod_shift_cnt, mod->d[msize-1]);
  if (mod_shift_cnt)
    _gcry_mpih_lshift (mp, mod->d, msize, mod_shift_cnt);
  else
...
/*cache*/    negative_result = (ep[0] & 1) & bsign;//negative_result = (ep[0] & 1) && bsign;
    ...
    if (W > 1)                  /* X := BASE^2 */
      mul_mod (xp, &xsize, bp, bsize, bp, bsize, mp, msize, &karactx, pub_branch4);
    base_u = precomp[0] = pub_l5; // mpi_alloc_limb_space (bsize, esec);
...
/*excluded*/    base_u = pub_l7;//base_u = mpi_alloc_limb_space (max_u_size, esec);
    ...
    while (j--)
      {
        mul_mod (xp, &xsize, rp, rsize, rp, rsize, mp, msize, &karactx, pub_branch15);
        tp = rp; rp = xp; xp = tp;
        rsize = xsize;
      }
