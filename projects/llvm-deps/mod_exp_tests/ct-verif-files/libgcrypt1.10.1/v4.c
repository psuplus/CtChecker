...
/*loop*/ /*+*/ MPN_NORMALIZE_1(ep, esize);
  W = 5 * (esize * BITS_PER_MPI_LIMB > 512) + 4 * (esize * BITS_PER_MPI_LIMB > 256 && esize * BITS_PER_MPI_LIMB <= 512) + 3 * (esize * BITS_PER_MPI_LIMB > 128 && esize * BITS_PER_MPI_LIMB <= 256) + 2 * (esize * BITS_PER_MPI_LIMB > 64 && esize * BITS_PER_MPI_LIMB <= 128) + 1 * (esize * BITS_PER_MPI_LIMB <= 64);
...
/*excluded*/               dummy++;//mpi_set_cond (&w, &u, j != 0);
    ...
    /* We shifted MOD, the modulo reduction argument, left
       ...
       Also make sure the result is put in RES->d (where it already
       might be, see above).  */
    if (pub_branch24)//if ( mod_shift_cnt )
      {
...
/*cache*/            dummy++;//            rp[rsize] = carry_limb;
    ...
    else if (pub_branch23) //else if (res->d != rp)
      {
