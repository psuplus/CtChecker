...
#include "v4_4_include/mpi-internal.h"
...
/*excluded*/    dummy++;//_gcry_mpih_mul_karatsuba_case (xp, rp, rsize, sp, ssize, karactx_p);

   if (pub_branch0)//if (rsize + ssize > msize)
    {
 ...
 */
void
_gcry_mpi_powm_algorithm (gcry_mpi_t res,
                          gcry_mpi_t base, gcry_mpi_t expo, gcry_mpi_t mod, int pub_int0,int pub_int1,int pub_int2,mpi_limb_t *pub_l0,mpi_limb_t *pub_l1,mpi_limb_t *pub_l2,mpi_limb_t *pub_l3,mpi_limb_t *pub_l4,mpi_limb_t *pub_l5,mpi_limb_t *pub_l6,mpi_limb_t *pub_l7,mpi_limb_t pub_l8,int pub_branch0,int pub_branch1,int pub_branch2,int pub_branch3,int pub_branch4,int pub_branch5,int pub_branch6,int pub_branch7,int pub_branch8,int pub_branch9,int pub_branch10,int pub_branch11,int pub_branch12,int pub_branch13,int pub_branch14,int pub_branch15,int pub_branch16,int pub_branch17,int pub_branch18,int pub_branch19)
{
  /* Pointer to the limbs of the arguments, their size and signs. */
  mpi_ptr_t  rp, ep, mp, bp;
  mpi_size_t esize, msize, bsize, rsize;
...
//         on if MOD equals 1.  */
//      res->nlimbs = (msize == 1 && mod->d[0] == 1) ? 0 : 1;
//      if (pub_branch0) ///*+*/if (res->nlimbs)
//        {
//          RESIZE_IF_NEEDED_0 (res, 1);
//          rp = res->d;
//          rp[0] = 1;
//        }
//      res->sign = 0;
...
//         (The quotient is (bsize - msize + 1) limbs.)  */
//      bp_nlimbs = bsec ? (bsize + 1):0;
///*excluded*/      bp = bp_marker = pub_l1;//bp = bp_marker = mpi_alloc_limb_space( bsize + 1, bsec );
//      MPN_COPY ( bp, base->d, bsize );
//      /* We don't care about the quotient, store it above the
//       * remainder, at BP + MSIZE.  */
...
//    if (W > 1)                  /* X := BASE^2 */
//      mul_mod (xp, &xsize, bp, bsize, bp, bsize, mp, msize, &karactx,pub_branch3);
...
//      {
//        rsize = msize;
//        /*loop*/ /*+*/ MPN_ZERO_2 (rp, rsize);
//      }
//    /*loop*/ /*+*/ MPN_COPY_2 ( rp, bp, bsize );
//
//    e = ep[i];
//    count_leading_zeros (c, e);
//    e = (e << c) << 1;
...
//          e0 = (e0 >> c0) >> 1;
//
//          j += W - c0; ///*+*/if(j)dummy++; /*loop*/ //for (j += W - c0; j >= 0; j--)
//            {
//
//              /*
//               *  base_u <= precomp[e0]
...
//               */
//              base_u_size = 0;
//              k = 0; if(1<< (W - 1))dummy++; /*loop*/ //for (k = 0; k < (1<< (W - 1)); k++)
//                {
...
//        }
//
//    if(pub_branch12)///*+*/if(j--)/*loop*/ //while (j--)
//      {
//        mul_mod (xp, &xsize, rp, rsize, rp, rsize, mp, msize, &karactx,pub_branch13);
//        tp = rp; rp = xp; xp = tp;
//        rsize = xsize;
//      }
//
...
//       Also make sure the result is put in RES->d (where it already
//       might be, see above).  */
//    if ( pub_branch14 )//if ( mod_shift_cnt )
//      {
...
//      }
//
//    if ( pub_branch16 )///*+*/if ( rsize >= msize )
//      {
...
//
//  /* Fixup for negative results.  */
//  if ( pub_branch18 )///*+*/if ( negative_result && rsize )
//    {
//      if ( pub_branch19 )///*+*/if ( mod_shift_cnt )
...
//                              pub_branch12,pub_branch13,pub_branch14,pub_branch15,pub_branch16,pub_branch17,pub_branch18,pub_branch19);
//    
//    
//}
///*************************/
