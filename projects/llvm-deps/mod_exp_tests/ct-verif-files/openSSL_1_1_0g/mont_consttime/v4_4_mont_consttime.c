...
/*X+x*/    if(pub_BRANCH0) {//if (!BN_is_odd(m)) {
...
/*X*/    bits = p->d[0];//bits = BN_num_bits(p);
    if(pub_BRANCH1) {///*+*/if (bits == 0) {
        /* x**0 mod 1 is still zero. */
/*X+x*/        if(pub_BRANCH2) {//if (BN_is_one(m)) {
            ret = 1;
    /*X*/            dummy++;//BN_zero(rr);
        } else {
...
/*X*/        if(pub_BRANCH3) {//if ((mont = BN_MONT_CTX_new()) == NULL)
                goto err;}
/*X+x*/        if(pub_BRANCH4) {//if (!BN_MONT_CTX_set(mont, m, ctx))
                goto err;}
    }
    ...
    window = (bits);///3*+*+*+*/window = BN_window_bits_for_ctime_exponent_size(bits);
#if defined(SPARC_T4_MONT)
     ...
     */
    numPowers = 1 << window;
    powerbufLen += numPowers;/*+*//*sizeof(m->d[0]) * (top * numPowers +
                                      ((2 * top) >
                                       numPowers ? (2 * top) : numPowers));*/
#ifdef alloca
    if (pub_BRANCH5)///*+*/if (powerbufLen < 3072)
        powerbufFree =
/*X*/            chr0;//alloca(powerbufLen + MOD_EXP_CTIME_MIN_CACHE_LINE_WIDTH);
    else
#endif
        powerbufFree = chr0;/*+*/ /*if ((powerbufFree =
...
#ifdef alloca
    if (pub_BRANCH6)///*+*/if (powerbufLen < 3072)
        powerbufFree = NULL;
#endif
...
#endif
/*X*/    if(pub_BRANCH8) {//if (!BN_to_montgomery(&tmp, BN_value_one(), mont, ctx))
            goto err;}

    /* prepare a^1 in Montgomery domain */
    if (pub_BRANCH9) {///*+*/if (a->neg || BN_ucmp_0(a, m, pub_BRANCH9) >= 0) {
/*X+x*/        if(pub_BRANCH10) {//if (!BN_mod(&am, a, m, ctx))
                goto err;}
/*X*/        if(pub_BRANCH11) {//if (!BN_to_montgomery(&am, &am, mont, ctx))
                goto err;}
/*X*/    } else if(pub_BRANCH12) {//if (!BN_to_montgomery(&am, a, mont, ctx))
            goto err;}
...
#endif
    {
        MOD_EXP_CTIME_COPY_TO_PREBUF(&tmp, top, powerbuf, 0, window);///*+*/if (!MOD_EXP_CTIME_COPY_TO_PREBUF(&tmp, top, powerbuf, 0, window))
            //goto err;
        MOD_EXP_CTIME_COPY_TO_PREBUF(&am, top, powerbuf, 1, window);///*+*/if (!MOD_EXP_CTIME_COPY_TO_PREBUF(&am, top, powerbuf, 1, window))
            //goto err;
         ...
         */
        if (pub_BRANCH13) {///*+*/if (window > 1) {
/*X*/            if(pub_BRANCH14) {//if (!BN_mod_mul_montgomery(&tmp, &am, &am, mont, ctx))
                    goto err;}
            MOD_EXP_CTIME_COPY_TO_PREBUF(&tmp, top, powerbuf, 2,
                                         window);///*+*/if (!MOD_EXP_CTIME_COPY_TO_PREBUF(&tmp, top, powerbuf, 2,
                                              //window))
                //goto err;
            i = 3; {///*+*/if(numPowers)dummy++; {/*loop*/ //for (i = 3; i < numPowers; i++) {
                /* Calculate a^i = a^(i-1) * a */
/*X*/                if(pub_BRANCH16) {//if (!BN_mod_mul_montgomery(&tmp, &am, &tmp, mont, ctx))
                        goto err;}
                MOD_EXP_CTIME_COPY_TO_PREBUF(&tmp, top, powerbuf, i,
                                             window);///*+*/if (!MOD_EXP_CTIME_COPY_TO_PREBUF(&tmp, top, powerbuf, i,
                                                  //window))
                    //goto err;
            }
        }
        ...
        MOD_EXP_CTIME_COPY_FROM_PREBUF_0(&tmp, top, powerbuf, wvalue,
                                       window, pub_BRANCH18);/*+*//*if (!MOD_EXP_CTIME_COPY_FROM_PREBUF(&tmp, top, powerbuf, wvalue,
                                            window))*/
            //goto err;
...
/*X*/                if(pub_BRANCH19) {//if (!BN_mod_mul_montgomery(&tmp, &tmp, &tmp, mont, ctx))
             ...
             */
            MOD_EXP_CTIME_COPY_FROM_PREBUF_0(&am, top, powerbuf, wvalue,
                                           window, pub_BRANCH21);/*+*//*if (!MOD_EXP_CTIME_COPY_FROM_PREBUF(&am, top, powerbuf, wvalue,
                                                window))*/
//                goto err;

            /* Multiply the result into the intermediate result */
/*X*/            if(pub_BRANCH22) {//if (!BN_mod_mul_montgomery(&tmp, &tmp, &am, mont, ctx))
                    goto err;}
        }
    }
