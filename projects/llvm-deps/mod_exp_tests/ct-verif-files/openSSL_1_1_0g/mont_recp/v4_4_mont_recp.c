 ...
 */
//#include "v4_4_mont_recp_lib/bn_lib.c"
...
/*X*/    bits = p->d[0];//bits = BN_num_bits(p);
    if (pub_BRANCH0) {///*+*/if (bits == 0) {
        /* x**0 mod 1 is still zero. */
/*X+x*/        if (pub_BRANCH1) {//if (BN_is_one(m)) {
            ret = 1;
/*X*/            dummy++;//BN_zero(r);
        } else {
/*X*/            ret = pub_VAL0;//ret = BN_one(r);
        }
        return ret;
    }
...
/*X*/    dummy++;//BN_RECP_CTX_init(&recp);
    if (m->neg) {
        /* ignore sign of 'm' */
/*X+x*/        if (pub_BRANCH2){//if (!BN_copy(aa, m))
                goto err;}
        aa->neg = 0;
/*X*/        if (pub_BRANCH3){//if (BN_RECP_CTX_set(&recp, aa, ctx) <= 0)
                goto err;}
    } else {
/*X+x*/        if (pub_BRANCH4){//if (BN_RECP_CTX_set(&recp, m, ctx) <= 0)
                goto err;}
    }

/*X+x*/    if (pub_BRANCH5){//if (!BN_nnmod(val[0], a, m, ctx))
        goto err;}               /* 1 */
...
/*X*/        dummy++;//BN_zero(r);
    ...
    window = bits &1;///3*+*+*+*/window = BN_window_bits_for_exponent_size(bits);
    if (pub_BRANCH7) {///*+*/if (window > 1) {
...
/*L*/    start = pub_VAL1;/*start = 1;                  /* This is used to avoid multiplication etc
...
/*X*/    if (pub_BRANCH10){//if (!BN_one(r))
            goto err;}

    {/*loop*/ //for (;;) {
        if (pub_BRANCH11) {///*+*/if (BN_is_bit_set_0(p, wstart, pub_BRANCH11) == 0) {
            if (!start) {
/*X*/                if (pub_BRANCH12){//if (!BN_mod_mul_reciprocal(r, r, r, &recp, ctx))
                        goto err;}}
            if (pub_BRANCH13)///*+*/if (wstart == 0)
                dummy++;//break;
            wstart--;
            dummy++;//continue;
        }
        ...
        i = window-1; {///*+*/if(window)dummy++; {/*loop*/ //for (i = 1; i < window; i++) {
            if (pub_BRANCH14)//if (wstart - i < 0)
                dummy++;//break;
            if (pub_BRANCH15) {///*+*/if (BN_is_bit_set_0(p, wstart - i, pub_BRANCH11)) {
                wvalue <<= (i - wend);
                wvalue |= 1;
                wend = i;
            }
        ...
        /* add the 'bytes above' */
        if (!start)
            i = 0; {///*+*/if(j)dummy++; {/*loop*/ //for (i = 0; i < j; i++) {
/*X*/                if (pub_BRANCH16){//if (!BN_mod_mul_reciprocal(r, r, r, &recp, ctx))
                        goto err;}
            }

        /* wvalue will be an odd number < 2^window */
/*X*/        if (pub_BRANCH17)//if (!BN_mod_mul_reciprocal(r, r, val[wvalue >> 1], &recp, ctx))
            goto err;
