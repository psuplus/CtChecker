...
/*X*/            ret = dummy++;////BN_one(rr);
        }
        return ret;
    }
    if (pub_BRANCH3) {///*+*/if (a == 0) {
/*X*/        dummy++;//BN_zero(rr);
        ret = 1;
        return ret;
    }
...
/*X*/    d = pub_BIGNUM0;//BN_CTX_get(ctx);
/*X*/    r = pub_BIGNUM1;//BN_CTX_get(ctx);
/*X*/    t = pub_BIGNUM2;//BN_CTX_get(ctx);
...
/*X*/        if (pub_BRANCH4) {//if ((mont = BN_MONT_CTX_new()) == NULL)
            goto err;}
/*X+x*/        if (pub_BRANCH5) {//if (!BN_MONT_CTX_set(mont, m, ctx))
            goto err;}
    }
    ...
    b = bits - 2; {///*+*/if(b)dummy++; {/*loop*/ //for (b = bits - 2; b >= 0; b--) {
        /* First, square r*w. */
        next_w = w * w;
        if (pub_BRANCH6) {//if ((next_w / w) != w) { /* overflow */
            if (r_is_one) {
/*X*/                if (pub_BRANCH7) {//if (!BN_TO_MONTGOMERY_WORD(r, w, mont))
                    goto err;}
                r_is_one = 0;
            } else {
/*X+x*/                if (pub_BRANCH8) {//if (!BN_MOD_MUL_WORD(r, w, m))
                    goto err;}
            }
            next_w = 1;
        }
...
/*X*/            if (pub_BRANCH9) {//if (!BN_mod_mul_montgomery(r, r, r, mont, ctx))
        ...
        if (pub_BRANCH11) {///*+*/if (BN_is_bit_set_0(p, b, pub_BRANCH10)) {
            next_w = w * a;
            if (pub_BRANCH12) {///*+*/if ((next_w / a) != w) { /* overflow */
                if (r_is_one) {
/*X*/                    if (pub_BRANCH13) {//if (!BN_TO_MONTGOMERY_WORD(r, w, mont))
                        goto err;}
                    r_is_one = 0;
                } else {
/*X+x*/                    if (pub_BRANCH14) {//if (!BN_MOD_MUL_WORD(r, w, m))
                        goto err;}
                }
                next_w = a;
            }
    ...
    /* Finally, set r:=r*w. */
    if (pub_BRANCH15) {///*+*/if (w != 1) {
        if (r_is_one) {
/*X*/            if (pub_BRANCH16) {//if (!BN_TO_MONTGOMERY_WORD(r, w, mont))
                goto err;}
            r_is_one = 0;
        } else {
/*X+x*/            if (pub_BRANCH17) {//if (!BN_MOD_MUL_WORD(r, w, m))
                goto err;}
        }
    }
