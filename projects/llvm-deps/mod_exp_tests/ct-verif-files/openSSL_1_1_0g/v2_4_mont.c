...
/*excluded*/    /*if (BN_get_flags(p, BN_FLG_CONSTTIME) != 0
            || BN_get_flags(a, BN_FLG_CONSTTIME) != 0
            || BN_get_flags(m, BN_FLG_CONSTTIME) != 0) {
...
/*excluded*/    bits = p->d[0];//bits = BN_num_bits(p);
    if (bits == 0) {
        /* x**0 mod 1 is still zero. */
/*excluded*/        if (pub_BRANCH2) {
            ret = 1;
            BN_zero(rr);
        } else {
            ret = BN_one(rr);
...
/*excluded*/    //BN_CTX_start(ctx);
/*excluded*/    d = pub_BIGNUM0;//d = BN_CTX_get(ctx);
/*excluded*/    r = pub_BIGNUM1;//r = BN_CTX_get(ctx);
/*excluded*/    val[0] = pub_BIGNUM2;//val[0] = BN_CTX_get(ctx);
    if (!d || !r || !val[0])
        goto err;
...
/*excluded*/        if (pub_BRANCH7)//if (!BN_nnmod(val[0], a, m, ctx))
        ...
        aa = a;
/*excluded*/    if (pub_BRANCH8) {//if (BN_is_zero(aa)) {
        BN_zero(rr);
        ret = 1;
        goto err;
    }
/*excluded*/    if (pub_BRANCH9)//if (!BN_to_montgomery(val[0], aa, mont, ctx))
        goto err;               /* 1 */
...
/*excluded*/    if (pub_BRANCH16)//if (!BN_to_montgomery(r, BN_value_one(), mont, ctx))
    ...
    for (;;) {
        if (BN_is_bit_set_0(p, wstart - i, pub_BRANCH26) == 0) {
            if (!start) {
/*excluded*/                if (pub_BRANCH18)//if (!BN_mod_mul_montgomery(r, r, r, mont, ctx))
                    goto err;
            }
            if (wstart == 0)
                dummy++;//break;
