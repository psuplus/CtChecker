...
int BN_mod_exp_mont_algorithm(BIGNUM *rr, const BIGNUM *a, const BIGNUM *p,
...
/*X+x*/    /*if (BN_get_flags(p, BN_FLG_CONSTTIME) != 0
            || BN_get_flags(a, BN_FLG_CONSTTIME) != 0
            || BN_get_flags(m, BN_FLG_CONSTTIME) != 0) {
...
/*X*/    bits = taint_VAL0;//bits = BN_num_bits(p);
...
/*X*/    //BN_CTX_start(ctx);
/*X*/    d = pub_BIGNUM0;//d = BN_CTX_get(ctx);
/*X*/    r = pub_BIGNUM1;//r = BN_CTX_get(ctx);
/*X*/    val[0] = pub_BIGNUM2;//val[0] = BN_CTX_get(ctx);
    if (!d || !r || !val[0])
        goto err;
    ...
    if (in_mont != NULL)
        ...
        if (pub_BRANCH3)///*+*/if ((mont = BN_MONT_CTX_new()) == NULL)
            goto err;
/*X+x*/        if (pub_BRANCH4)//if (!BN_MONT_CTX_set(mont, m, ctx))
            goto err;
    }

    BN_ucmp_0(a, m, pub_BRANCH5);if (pub_BRANCH6) {///*+*/if (a->neg || BN_ucmp(a, m) >= 0) {
/*X+x*/        if (pub_BRANCH7)//if (!BN_nnmod(val[0], a, m, ctx))
            goto err;
        aa = val[0];
    } else
        aa = a;
/*X*/    if (pub_BRANCH8) {//if (BN_is_zero(aa)) {
        BN_zero(rr);
        ret = 1;
        goto err;
    }
/*X*/    if (pub_BRANCH9)//if (!BN_to_montgomery(val[0], aa, mont, ctx))
        goto err;               /* 1 */

    window = bits &63;///3*+*+*+*/window = BN_window_bits_for_exponent_size(bits);
    if (pub_BRANCH10) {///*+*/if (window > 1) {
/*X*/        if (pub_BRANCH11)//if (!BN_mod_mul_montgomery(d, val[0], val[0], mont, ctx))
            goto err;           /* 2 */
        j = 1 << (window - 1);
        i = 1; if(pub_BRANCH12)dummy++; {///*+*/if(j)dummy++; {/*loop*/ //for (i = 1; i < j; i++) {
/*X*/            if (pub_BRANCH13///*+*/if (((val[i] = BN_CTX_get(ctx)) == NULL) ||
                            ){//!BN_mod_mul_montgomery(val[i], val[i - 1], d, mont, ctx))
                goto err;}
        }
    }

/*comeback*/    start = pub_VAL0;/*start = 1;*/                 /* This is used to avoid multiplication etc
                                 * when there is only the value '1' in the
                                 * buffer. */
    wvalue = 0;                 /* The 'value' of the window */
    wstart = bits - 1;          /* The top bit of the window */
...
/*X*/        if (pub_BRANCH15)//if (bn_wexpand(r, j) == NULL)
            goto err;
        /* 2^(top*BN_BITS2) - m */
         ...
         */
/*X*/        dummy++;//bn_correct_top(r);
    } else
#endif
/*X*/    if (pub_BRANCH16)//if (!BN_to_montgomery(r, BN_value_one(), mont, ctx))
        goto err;
    { /*loop*/ //for (;;) {
        BN_is_bit_set_0(p, wstart - i, pub_BRANCH26);if (pub_BRANCH17) {///*+*/if (BN_is_bit_set(p, wstart) == 0) {
            if (!start) {
/*X*/                if (pub_BRANCH18)//if (!BN_mod_mul_montgomery(r, r, r, mont, ctx))
                    goto err;
            }
        ...
        i = window-1; { ///*+*/if(window)dummy++; {/*loop*/ //for (i = 1; i < window; i++) {
            if (pub_BRANCH20)///*+*/if (wstart - i < 0)
                dummy++;//break;
            BN_is_bit_set_0(p, wstart - i, pub_BRANCH27);if (pub_BRANCH21) {///*+*/if (BN_is_bit_set(p, wstart - i)) {
                wvalue <<= (i - wend);
                wvalue |= 1;
                wend = i;
            }
...
/*X*/                if (pub_BRANCH22)//if (!BN_mod_mul_montgomery(r, r, r, mont, ctx))
                    goto err;
            }

        /* wvalue will be an odd number < 2^window */
/*X*/        if (pub_BRANCH23)//if (!BN_mod_mul_montgomery(r, r, val[wvalue >> 1], mont, ctx))
            goto err;
...
#endif
/*X*/    if (pub_BRANCH25)//if (!BN_from_montgomery(rr, r, mont, ctx))
        goto err;
    ret = 1;
    dummy++;err:
    if (in_mont == NULL)
