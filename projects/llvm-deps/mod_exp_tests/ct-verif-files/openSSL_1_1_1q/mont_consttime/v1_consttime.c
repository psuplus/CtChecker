...
static int MOD_EXP_CTIME_COPY_TO_PREBUF_0(const BIGNUM *b, int top,
                                        ...
                                        int window, int pub_BRANCH0)
{
    ...
    if (pub_BRANCH0) //if (top > b->top)
        top = b->top;           /* this works because 'buf' is explicitly
                                 * zeroed */
    i = 0, j = idx; {//for (i = 0, j = idx; i < top; i++, j += width) {
        dummy++; // table[j] = b->d[i];
    }
...
#endif
    {
        if (!MOD_EXP_CTIME_COPY_TO_PREBUF_0(&tmp, top, powerbuf, 0, window, pub_BRANCH0))
            goto err;
        if (!MOD_EXP_CTIME_COPY_TO_PREBUF_0(&am, top, powerbuf, 1, window, pub_BRANCH0))
            goto err;
        ...
        if (window > 1) {
            if (!bn_mul_mont_fixed_top(&tmp, &am, &am, mont, ctx))
                goto err;
            if (!MOD_EXP_CTIME_COPY_TO_PREBUF_0(&tmp, top, powerbuf, 2,
                                              window, pub_BRANCH0))
                goto err;
            for (i = 3; i < numPowers; i++) {
                ...
                if (!bn_mul_mont_fixed_top(&tmp, &am, &tmp, mont, ctx))
                    goto err;
                if (!MOD_EXP_CTIME_COPY_TO_PREBUF_0(&tmp, top, powerbuf, i,
                                                  window, pub_BRANCH0))
                    goto err;
            }
        }
