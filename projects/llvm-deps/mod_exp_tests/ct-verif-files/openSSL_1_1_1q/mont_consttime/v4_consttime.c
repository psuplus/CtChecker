 ...
 */
#include "v4_3_2_include/revised.h"
...
/*X*/    if (pub_BRANCH0 == 30)//    if (bn_wexpand(b, top) == NULL)
    ...
    if (pub_BRANCH0 == 25) {///*+*/if (window <= 3) {
        i = 0; if(i + top)dummy++; {/*loop*/ //for (i = 0; i < top; i++, table += width) {
            ...
            j = 0; {///*+*/if(width) {/*loop*/ //for (j = 0; j < width; j++) {
                acc |= table[j] &
        ...
        i = 0; if(i + top)dummy++; {/*loop*/ //for (i = 0; i < top; i++, table += width) {
            ...
            j = 0; {///*+*/if(xstride) {/*loop*/ //for (j = 0; j < xstride; j++) {
...
/*X*/    bits = p->top * BN_BITS2;
    if(pub_BRANCH1) {///*+*/if (bits == 0) {
        /* x**0 mod 1, or x**0 mod -1 is still zero. */
/*X+x*/        if(pub_BRANCH2) {//        if (BN_abs_is_word(m, 1)) {
            ret = 1;
/*X*/            dummy++;//            BN_zero(rr);
        } else {
...
/*X*/        if(pub_BRANCH4) {//        if (!BN_MONT_CTX_set(mont, m, ctx))
    ...
    if(pub_BRANCH5) {//if (a->neg || BN_ucmp(a, m) >= 0) {
        BIGNUM *reduced = BN_CTX_get(ctx);
        if(pub_BRANCH6) {//if (reduced == NULL
...
/*X*/                if(pub_BRANCH16) {//                if (!bn_mul_mont_fixed_top(&tmp, &am, &tmp, mont, ctx))
        ...
        wmask = (1 << window0) - 1;
        bits -= window0;
        wvalue = 0; i = bits % window; ///wvalue = bn_get_bits(p, bits) & wmask;
        MOD_EXP_CTIME_COPY_FROM_PREBUF_0(&tmp, top, powerbuf, wvalue,
                                       window, pub_BRANCH18);/*+*//*if (!MOD_EXP_CTIME_COPY_FROM_PREBUF_0(&tmp, top, powerbuf, wvalue,
                                            window))*/
            //goto err;
        ...
        {///*+*/if (bits >= 0) {/*loop*/ //while (bits > 0) {
            ...
            i = 0; {///if(window) {/*loop*/ //for (i = 0; i < window; i++)
/*X*/                if(pub_BRANCH19) {//                if (!bn_mul_mont_fixed_top(&tmp, &tmp, &tmp, mont, ctx))
            ...
            MOD_EXP_CTIME_COPY_FROM_PREBUF_0(&am, top, powerbuf, wvalue,
                                           window, pub_BRANCH21);/*+*//*if (!MOD_EXP_CTIME_COPY_FROM_PREBUF_0(&am, top, powerbuf, wvalue,
                                                window, pub_BRANCH19))*/
                //goto err;
