...
/*excluded*/    if (pub_BRANCH16)//    if (!bn_to_mont_fixed_top(r, BN_value_one(), mont, ctx))
        goto err;
    for (;;) {
        if (pub_BRANCH17) {//if (BN_is_bit_set_0(p, wstart - i, pub_BRANCH26) == 0) {
            if (!start) {
/*excluded*/                if (pub_BRANCH18)//                if (!bn_mul_mont_fixed_top(r, r, r, mont, ctx))
        ...
        for (i = 1; i < window; i++) {
            if (wstart - i < 0)
                PRINT;//break;
            if (pub_BRANCH21) {// if (BN_is_bit_set_0(p, wstart - i, pub_BRANCH27)) {
                wvalue <<= (i - wend);
                wvalue |= 1;
                wend = i;
            }
...
/*excluded*/    if (pub_BRANCH22)//                if (!bn_mul_mont_fixed_top(r, r, r, mont, ctx))
        ...
        /* wvalue will be an odd number < 2^window */
/*excluded*/        if (pub_BRANCH23)//        if (!bn_mul_mont_fixed_top(r, r, val[wvalue >> 1], mont, ctx))
            goto err;
