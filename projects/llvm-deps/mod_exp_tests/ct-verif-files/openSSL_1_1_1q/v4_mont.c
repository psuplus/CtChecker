...
/*X+x*/    /*    if (BN_get_flags(p, BN_FLG_CONSTTIME) != 0
    ...
    BN_ucmp_0(a, m, pub_BRANCH5);if (pub_BRANCH6) {///*+*/if (a->neg || BN_ucmp_0(a, m, pub_BRANCH5) >= 0) {
	/*X+x*/        if (pub_BRANCH7)//        if (!BN_nnmod(val[0], a, m, ctx))
            goto err;
        aa = val[0];
    } else
        aa = a;
	/*X*/    if (pub_BRANCH8) {//    if (!bn_to_mont_fixed_top(val[0], aa, mont, ctx))
        goto err;}               /* 1 */

    window = bits &63;///3*+*+*+*/window = BN_window_bits_for_exponent_size(bits);
    if (pub_BRANCH10) {///*+*/if (window > 1) {
	/*X*/        if (pub_BRANCH11)//        if (!bn_mul_mont_fixed_top(d, val[0], val[0], mont, ctx))
            goto err;           /* 2 */
        j = 1 << (window - 1);
...
/*X*/        if (pub_BRANCH15)//        if (bn_wexpand(r, j) == NULL)
        ...
        r->top = j;
	/*X*/        dummy++;//        r->flags |= BN_FLG_FIXED_TOP;
    } else
#endif
	/*X*/    if (pub_BRANCH16)//    if (!bn_to_mont_fixed_top(r, BN_value_one(), mont, ctx))
        goto err;
    { /*loop*/ //for (;;) {
        BN_is_bit_set_0(p, wstart - i, pub_BRANCH26);if (pub_BRANCH17) {///*+*/if (BN_is_bit_set_0(p, wstart - i, pub_BRANCH26) == 0) {
            if (!start) {
	/*X*/               if (pub_BRANCH18)//                if (!bn_mul_mont_fixed_top(r, r, r, mont, ctx))
                    goto err;
            }
...
/*X*/                if (pub_BRANCH22)//                if (!bn_mul_mont_fixed_top(r, r, r, mont, ctx))
                    goto err;
            }
        }
        /* wvalue will be an odd number < 2^window */
/*X*/        if (pub_BRANCH23)//        if (!bn_mul_mont_fixed_top(r, r, val[wvalue >> 1], mont, ctx))
            goto err;
