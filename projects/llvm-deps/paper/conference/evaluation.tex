...
its shadow counterpart by \texttt{assume}. Then, every assignment in \textit{P} is associated with an assignment on shadow variables. When a branch test or array indexing is reached, 
\texttt{shadow\_ok} will be updated by adding asserting that the branch condition or 
index is the same with its shadow counterpart. At the end of the program, Ct-Verif
...
shadow_ok := shadow_ok && (i == i_shadow);
e = a[i];
e_shadow = a_shadow[i_shadow];
