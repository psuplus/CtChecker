...
SMACK verification tool~\cite{} as a front-end to analyze the semantics of IR
code and generate verification conditions which is further analyzed by
Boogie~\cite{}. We emphasize that both SMACK and \toolname{} uses the DSA
...
an example of how we rewrite code} and re-ran Ct-Verif on the revised code.
...
to it. CtChecker will output the exact lines that are positive. On the other
hand, Ct-verif doesn't tell which line is positive exactly. Therefore, in order
to know which line is reported by Ct-verif, we feed the program line by line.
At the beginning, the first line of code is input to Ct-verif, if it is not
reported positive, we add the next line, otherwise, we change the branch
condition to a public value and add the next line. However, this introduces a
problem. \danfeng{I don't understand the following sentence. Try to rephrase
it} To maintain consistency among libraries under investigation, we include
certain functions with same purpose, such as decryption.  So, there will be
some positive lines out if this scope due to function calls.  When operating
Ct-verif in an aforementioned manner, we have to change some of the out of
...
The second version (excluded) was constructed from the first, the reasons are
...
The third version (removed) was created from the second to remove all erroneous
