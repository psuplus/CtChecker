...
\begin{table*}[!t]
  ...
  \begin{tabular}{@{}lcrrrrcrrrr@{}}
    ...
    \bottomrule
  \end{tabular}
\caption{ The baseline file accommodated the excluded source.  Loop positives were removed from the baseline in a separate file. A new file removed the remaining positives.}
\label{tbl:runtimes}
\end{table*}

In this section, we discuss another approach to verify constant time code, and
compare the precision and accuracy between it and CtChecker.  Our goal is to
perform a reasonable comparison that focuses on the advantages of each method,
and not the engineering details associated with implementing them. We consider
...
automatically using a theorem solver.
...
Ct-verif and CtChecker. We verified the accuracy of CtChecker on the crypto
libraries by checking that all true positives reported by Ct-verif were also
reported by CtChecker.  Next, we compared the precision of the two tools; we
looked at positives reported by only one of the two tools.  We avoided certain
...
Ct-verif, its practical interpretation, must be analyzed carefully.  To perform
...
First, a baseline version was created to accommodate minor limitations or
differences between the tools, such as the way in which they operate with
some undefined values. … Assignment statements with excluded source were
rewritten to assign the variable to a public or private input, depending on
the function return value. If any of the function’s parameters were tainted,
we conservatively tainted its return value. Thus, a function call inside of
a branch condition was counted as a positive when any of its parameters
were tainted.
...
same information as the loop: a \codevar{if (e) then} structure where
\codevar{e} contains the loop bounds. Ct-verif supports an annotation
for a static number of values to be marked as public.  Variable length
arrays are not supported, so variable array element accesses needed to
be removed. Every array index was fixed within the bounds of the array.

The third version was created from the second to remove each erroneous line
reported by Ct-verif, resulting in a subset of the original file that was
free of any constant time violation. This final version allowed us to
evaluate the precision of CtChecker in another way; any positive reported
on this version was exclusive to CtChecker, and expected to be a false one.

To clarify, we see 0 positives reported in version 4 by Ct-verif since we
replaced them with some constant time code.  Thus, any positives on this
version were reported exclusively by CtChecker. Thus, any positives on this 
version were reported exclusively by CtChecker. The positives reported in
the final version by CtChecker were the result of two design choices:
making CtChecker a flow insensitive analysis, and taint pointers rather
than a range of values. The number illustrates the tradeoff in the method
to taint arrays, and is favorable for CtChecker. Overall, CtChecker showed
equal or improved precision over Ct-verif when disregarding significant
limitations, such as loop invariants. This result is apparent in the
difference between the number of positives reported by each tool in version
3. CtChecker showed improved precision over Ct-verif in version 2, a result
apparent in the difference between the number of positives reported by each
tool in version 2.


The results of each version are helpful in evaluating the distinct advantages
of each analysis.  Of course, engineering issues are not the focus of the
comparison, so they were avoided in the following ways.
A consistent procedure was used to construct subsets of the crypto libraries,
wherein each subset is a modified version of the original file. Three versions
are created, and each is a proper subset of the file before it.
...
the system stopped execution during its product program verification.  Constant
time violations that occurred first in sequence were reported, but
identification of violations that occurred afterwards was not guaranteed. The
product construction in Almeida’s work dictates that an execution will reach
subsequent state if and only if the two executions leak the same observation
sequence. In other words, execution will stop once a violation is found.  An
implementation that records a history of errors could point to all violations.
