...
line with some constant time code and re-ran the tool on the revised code that
preserves information flow\danfeng{add an example of how we rewrite code} and
use Ct-Verif on the revised code. During this iterative process, we record all
errornous lines reported by Ct-Verif and compare them with the locations found
by \toolname{}.
...
comparing the precision of the two tools against one another. There are
three versions (baseline, excluded and removed), each of which was
developed from the version number before it.

First, a baseline version was created to bridge the gap between
the tools in terms of how they operate. To run CtChecker, we feed the whole program
to it. CtChecker will output the exact lines that are positive. On the other
hand, Ct-verif doesn't tell which line is positive exactly. Therefore, in
order to know which line is reported by Ct-verif, we feed the program line
by line. At the beginning, the first line of code is input to Ct-verif, if
it is not reported positive, we add the next line, otherwise, we change 
the branch condition to a public value and add the next line. However, this
introduces a problem. To maintain consistency among libraries under investigation,
we include certain functions with same purpose, such as decryption.
So, there will be some positive lines out if this scope due to function calls.
When operating Ct-verif in an aforementioned manner, we have to change some of
the out of scope code. Even though we carefully do the changes so that the 
information flow remains the same, there still a chance the flow might be
changed. For a fair comparison, in version 1, those positives out of scope 
will remain changed for CtChecker.
...
and feed into Ct-verif, the tool ran out of memory, for which we remove all
the function calls without source. The second reason is to make the comparison
more meaningful. Both tools taint a function call without source conservatively.
This leads to a great portion of false positives. These positives does not reflect
how CtChecker performs since it can reduce such positives by simply including full
source. To this end, ruling out this kind of false positives is necessary for a
