...
executions. \danfeng{add an example here} \textbf{Figure~\ref{code:ct_verif_P} and 
\ref{code:ct_verif_product} show how Ct-Verif construct program \textit{Q} from \textit{P}.
In general, Ct-Verif doubles the number of variables and instructions. Every variable
will have a shadow counterpart. When an operation is performed on the original
variables, a same operation will be carried out on their shadow counterparts.
At the beginning, Ct-Verif adds a variable called \texttt{shadow\_ok} and initializes
it to be \texttt{true}. All the public variables are guaranteed to be the same with 
its shadow counterpart by \texttt{assume}. Then, every assignment will be doubled
for a shadow assignment. When a branch test or array indexing is reached, 
\texttt{shadow\_ok} will be updated by adding whether the branch condition or 
index is the same with its shadow counterpart. At the end of the program, Ct-Verif
checks if \texttt{shadow\_ok} is \texttt{true}. If it is, all the branch
conditions and array indices are not dependent on sensitive information, meaning the 
original program is secure in terms of information flow; otherwise, the program
has information flow vulnerabilities.}
...
assert shadow_ok;
    \end{lstlisting}
    \caption{Program Q constructed by Ct-Verif}
...
line with some constant time code that preserves information flow \danfeng{add
an example of how we rewrite code} and re-ran Ct-Verif on the revised code. \textbf{For
example, in Figure~\ref{code:libgcrypt_limb_processing}, \texttt{e} is tainted at 
the first line and then used in the branch condition at line 4, which makes this 
branch vulnerable. Ct-Verif will report an error when we approach this line with
the aforementioned manner. We record this line and rewrite it with a condition that
is not affected by sensitive information, i.e., \texttt{pub\_branch8}.}
During this iterative process, we record all errornous lines reported by
Ct-Verif and compare them with the ones found by \toolname{}.
...
previous paragraph} To run CtChecker, we feed the whole program
to it. CtChecker will output the exact lines that are positive. This difference in
operation introduces a problem. \danfeng{I don't understand the following sentence. Try to rephrase
it} \textbf{We exploit both tools on four cryptography libraries, among them, we feed
part of the code that achieves the same functionality, such as encryption. Positives 
reported outside of these functions are deemed as out of scope code.} These positives are
due to function calls. When operating Ct-verif in an aforementioned manner,
we have to change some of the out of scope code\danfeng{why there are out of scope code?}. 
...
First, a \emph{baseline version} was created to bridge the gap between the
tools in terms of how they operate. \textbf{This version corresponds to the column with 
header "WL/FS/FlS" in Table~\ref{tbl:overall-feature-benchmark-new} except for
BearSSL corresponding to column "WL/FS/FlS/SRC". The reason why we only
include full source for BearSSL is that its code base is small and can be verified
by Ct-Verif. Other libraries with full source will make Ct-Verif run out of memory,
so we only include the source code of the functions we are analyzing for other libraries. From the 
result, one can observe there are small differences in the number of positives for
\toolname~compared to Table~\ref{tbl:overall-feature-benchmark-new}. This is an adjustment due to the different ways to operate. Ct-Verif 
would report each line once while \toolname~might report one line more than once
since there might be multiple vulnerable branch conditions in one line. So for
those lines that are reported by \toolname~more than once, we only count once
in this comparison. Also, for the aforementioned "out of scope" code, even 
though we carefully do the changes so that the information flow remains the same,
there still a chance the flow might be changed. For a fair comparison,
in version 1, those positives out of scope will remain changed for \toolname.}
 

\danfeng{here, we should explain what's the
difference between baseline and the code that we analyzed in Table 1 and 2,
something similar to 4.4.2. We should update 4.4.2 according to what Sixuan did
and merge 4.4.2 to this paragraph.}

The second version (excluded-undefined-function) was constructed from the first, the reasons are
twofold. First reason is to accommodate the difference of how the tools treat
excluded source. The bottom line is, in this version, we should have no function
calls whose source is excluded, which could be achieved by either adding all the
source or removing all the excluded function calls. So, version 1 and version 2 of
BearSSL are the same since Ct-verif runs without problem on it.
For other three libraries that could make Ct-Verif run out of memory\danfeng{I'm confused. Does
the description belong to baseline? If not, why baseline does not say out of
memory?}, we remove all the function calls without source. The second
reason is to make the comparison more meaningful. Both tools taint a function
call without source conservatively.  This leads to a great portion of false
positives. These positives does not reflect how \toolname~performs since it can
reduce such positives by simply including full source. To this end, ruling out
this kind of false positives is necessary for a meaningful
comparison.\danfeng{there used to be a no-loop version. Is it still there?}
\sixuan{The no-loop version has been removed since it gives Ct-Verif too much credit.}

The third version (removed-Ct-Verif) was created from the second to remove all erroneous
lines reported by Ct-verif, resulting in a subset of the original file
...
\danfeng{Similar to the third version, we need a fouth version where we remove
all errors reported by our tool and see if Ct-Verif reports any positive}

\textbf{Similar to the third version, we add a fourth version (removed-\toolname) 
also from version 2, where all the lines reported by \toolname~are removed. The
modified files are fed into Ct-Verif. All the lines reported this time are
exclusive to Ct-Verif and also false positives.}
