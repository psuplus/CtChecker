...
\begin{table*}
	...
	% \begin{tabular}{|p{3.5cm}|p{1.5cm}|p{1.5cm}|p{1.5cm}|p{1.5cm}|p{1.5cm}|p{1.5cm}|p{1.5cm}|p{2.5cm}| p{2.5cm}|}
		\begin{tabular}{@{}lrrrrrrrrrrrr@{}}
			...
			%\bottomrule
		\end{tabular}
		\caption{Number of Warnings based on Features (Without-whitelist)}
		\label{tbl:overall-feature-benchmark-without-wl}
\end{table*}
...
self-composition technique~\cite{} to convert the constant-time property into a
...
Ct-Verif is that it is also built on top of the DSA~\cite{dsa} points-to
...
executions. \danfeng{add an example here} Theoretically, the reduction is sound
...
Simialr to \toolname{}, Ct-Verif analyzes LLVM IR code. Moreover, it uses the
...
reported outside of these functions are deemed as out of scope code.}
\sout{To maintain consistency among libraries under investigation, we include
certain functions with same purpose, such as decryption.}  These positives are due to function calls. 
When operating Ct-verif in an aforementioned manner, we have to change some of the out of
scope code\danfeng{why there are out of scope code?}. Even though we carefully
do the changes so that the information flow remains the same, there still a
chance the flow might be changed. For a fair comparison, in version 1, those
positives out of scope will remain changed for CtChecker.
...
tools in terms of how they operate.  \danfeng{here, we should explain what's the
...
source or removing all the excluded function calls. As mentioned before, the
consistency among libraries requires us to include source with same purpose, which
results in a full source incorporation of BearSSL. So, version 1 and version 2 of
BearSSL are the same. Since the code base of BearSSL is small, Ct-verif runs 
without problem. For other three libraries, when we try to include full source
and feed into Ct-verif, the tool ran out of memory\danfeng{I'm confused. Does
the description belong to baseline? If not, why baseline does not say out of
memory?}, for which we remove all the function calls without source. The second
...
positives. These positives does not reflect how CtChecker performs since it can
