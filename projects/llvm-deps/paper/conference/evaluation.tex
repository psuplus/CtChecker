...
<@\textcolor{red}{if (BN\_is\_bit\_set(p, b))}@> {
  ...
  The sliding window attacks from the other libraries were also able to be used
  in earlier version of OpenSSL. The function \codefn{bn\_mod\_exp\_simple} is
  no longer reachable but the code is specified as an entry point the results
  confirm other findings. Line 7 in figure \ref{openssl:simple} is reported
...
\begin{table*}[!t]
  ...
  \begin{tabular}{@{}lcrrrrcrrrr@{}}
    ...
    \cmidrule{3-6} \cmidrule{8-11}
    Library&& Baseline(v2) & No Loops(v3) & Removed(v4) & Diff && Baseline(v2) & No Loops(v3) & Removed(v4) & Diff\\
    \midrule
    ...
    \bottomrule
  \end{tabular}
\caption{ The baseline file accommodated the excluded
source.  Loop positives were removed from the baseline in a separate file. A new file removed the remaining positives.}
\label{tbl:runtimes}
\end{table*}

The paper of Almeida et al. uses an approach based on a reduction of the security of a program \textit{P} to the
assertion-safety of a program \textit{Q} \cite{almeida2016verifying}. The reduction is inspired by prior work on
self-composition and product programs.  A product program \textit{Q} verifies the security
of \textit{P} by simulating two executions in lockstep.   It asserts the equality of each
public input with its renamed copy at each branching instruction.  public outputs
are not taken into consideration, and the technique constructs an output-insensitive
product program.  Theoretically, the approach is sound and complete in that all
safe programs have a correct security verdict, and all unsafe programs have a
correct insecurity verdict.
...
There are three main ways we wanted to compare the precision and accuracy of
Ct-verif and CtChecker. We verified the accuracy of CtChecker on the crypto libraries
by checking that all true positives reported by Ct-verif
were also reported by CtChecker.  Next, we compared the precision of the two tools;
we looked at positives reported by only one of the two tools.  We avoided certain
engineering limitations in Ct-verif, such as loop invariants.

Ct-verif is flow sensitive; the order of statements in a program may affect the
analysis.  CtChecker's flow insensitive  is one cause of false

\subsubsection{Limitations}
Although the approach is theoretically sound and complete, the results of a
practical interpretation must be analyzed carefully.  To perform a direct
comparison, we avoid key limitations not present in CtChecker by revising the
analyzed source to accommodate Ct-verif's technical flaws.  For instance,
for-loops and non-static length arrays are not supported.
...
to infer that the memory accesses are in the range of public values.  Although
...
revisions made to avoid a for-loop and variable array access in \codefn{MPN\_NORMALIZE}
...
\sout{Although some examples in Almeida’s work contain thousands of lines of code,
they are relatively simple and not limited in any of the ways discussed (e.g.
figure 12 in their paper requires a manual loop invariant).  Code refactoring
to accommodate these limitations is an arduous task in advanced libraries.
Consider the complex function decomposition in mul\_mod from Libgcrypt, which
is called five times in \_gcry\_mpi\_powm.  When the entire source is not linked,
values computed from function calls outside of the source are undefined and
result in an unsafe execution of Q.  To eliminate this limitation requires
revision when an undefined value decides a branch.  Nevertheless, we managed
to verify [our] results.}

To avoid the previously described limitations when comparing our results, we constructed three new
subsets of the source to analyze different versions of the same code.  We saw
consistent results when applying the following
revisions. First, a baseline version was created to exclude source from separate
files. Assignment statements with
excluded source were rewritten to assign the variable to an public or private
input depending on the source. The second version was constructed from the first;
loops were removed and replaced by an \codevar{if (e) then}
structure where \codevar{e} contains the
loop bounds.  For each array, array indices were fixed within the bounds of the array since
only static sized arrays are supported. A third version was created from the second to remove erroneous lines reported by
Ct-verif. After refactoring the source to accommodate the limitations, Ct-verif exhibited
consistent behavior.

\sout{By annotating all input values, we can create a trivial
case for it to verify.  This ensures that the limitations described have been
avoided, and that the results are meaningful when analyzing buggy code.}

The implementation did not provide results for the entire source analyzed since
the system stopped execution during its product program verification.  Constant time
violations that occurred first in sequence were reported, but identification of
violations that occurred afterwards was not guaranteed.   The product
construction in Almeida’s work dictates that an execution will reach subsequent
state if and only if the two executions leak the same observation sequence.
In other words, execution will stop once a violation is found.  An implementation that records a history of errors could
point to all violations.
...
Ct-verif correctly reported all high-risk errors from the cryptography
libraries.  To compare the precision of the two tools against one another, they
were run on several versions of the cryptography algorithm.  There are three
versions, each of which was developed from the last.  The first file is the
original file modified to accomodate excluded source code.  The second removed
loops and other limitations such as array indexing.  The third and final version
removed all positives reported by Ct-verif.
...
tmp.top = top;
\end{lstlisting}
\caption{OpenSSL 1.1.0g - bn\_exp.c lines 741-744}
\label{code:libgcrypt_normalize}
\end{figure}

\subsubsection{Conclusion}
A direct comparison of precision would focus on two results: the number of
positives reported while disregarding limitations, and the number of positives
reported due to \sout{analysis design choices}. The number of positives in
\sout{version 2} and \sout{version 1} for Ct-verif and CtChecker, respectively, are
interpreted as the tools' best attempt at a taint analysis.  Positives
reported by CtChecker in version 3 are false positives caused by one of two
things: a flow insensitive analysis or the tainting procedure.
Ct-verif is a flow sensitive analysis and taints a range of values in the array,
leaving its pointer untainted. It correctly verifies line 742 in figure \sout{30},
a false positive reported by CtChecker.
...
\begin{figure}[h!]
\begin{lstlisting}
<@\textcolor{red}{for( i = 0; i < nbits; i++ )}@>
{
    MBEDTLS_MPI_CHK( mpi_montmul( X, X, N, mm, &T ) );
