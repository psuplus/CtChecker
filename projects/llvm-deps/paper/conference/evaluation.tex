...
\subsection{Comparision with Ct-Verif}
...
To verify constant-time property, one popular approach is to use the
self-composition technique~\cite{barthe2011secure} to convert the constant-time property into a
classical program verification problem. To compare static information flow
analysis apporach and self-compositoin approach, we compare \toolname{} with
Ct-Verif~\cite{almeida2016verifying}, one of the first tools that verify
constant-time properties via self-composition. One reason of comparing with
Ct-Verif is that it is also built on top of the DSA~\cite{lattner2007making} points-to
ananalysis; hence, the comparison focuses on the advantages of each approach,
rather than some engineering details, such as the difference in the points-to
analysis, in their implementations.

\subsubsection{Background}

Ct-Verif~\cite{almeida2016verifying} reduces the constant-time property of a
program \textit{P} to the assertion-safety of a program \textit{Q}. The
reduction is inspired by prior work on self-composition and product programs.
In particular, Ct-Verif simulates two program executions (with different secret
inputs) in lockstep.  It assures constant-time property by assuring that all
branch conditions and array indexes are identical across the two simulated
executions. 

Figure~\ref{code:ct_verif_product} illustrates how Ct-Verif construct program
\textit{Q} from \textit{P}.  In general, Ct-Verif doubles the number of
variables and instructions. Every variable in \textit{P} is associated with its
shadow counterpart. When an operation is performed on the original
variables, the same operation will be carried out on their shadow counterparts.
At the beginning, Ct-Verif adds a variable called \texttt{shadow\_ok} and initializes
it to be \texttt{true}. All public variables are assumed to be the same with 
its shadow counterpart by \texttt{assume}. Then, every assignment in \textit{P} is 
associated with an assignment on shadow variables. When a branch test or array indexing is reached, 
\texttt{shadow\_ok} will be updated by adding asserting that the branch condition or 
accessed address is the same with its shadow counterpart. At the end of the program, Ct-Verif
checks if \texttt{shadow\_ok} is \texttt{true}. If it is, the original program
is constant-time; otherwise, side channels are detected.
...
e = a[i];
    ...
    \begin{lstlisting}[mathescape=true]
// transformed program Q
shadow_ok := true;
assume $\forall$ x $\in$ X, x == x_shadow;
a := [0, 1, 2, 3];
a_shadow := [0, 1, 2, 3];
b := c + d;
b_shadow := c_shadow + d_shadow;

shadow_ok := shadow_ok && (b == b_shadow);
if (b) {
    b := b + 1;
    b_shadow := b_shadow + 1;
}
...
addr := a + i * 4;
addr_shadow := a_shadow + i_shadow * 4;
shadow_ok := shadow_ok && (addr == addr_shadow);
e = *addr;
e_shadow = *addr_shadow;

assert shadow_ok;
    \end{lstlisting}

    \caption{Program P and its transformed program being verified by Ct-Verif.}
    \label{code:ct_verif_product}
\end{figure}

Theoretically, the reduction is sound
and complete in that all safe programs have a correct security verdict, and all
unsafe programs have a correct insecurity verdict. But in practice, verifying
the recuded code is typically incomplete, due to loops in the code as well as
incompleteness of SMT solvers. 

Similar to \toolname{}, Ct-Verif analyzes LLVM IR code. Moreover, it uses the
SMACK verification tool~\cite{carter2016smack} as a front-end to analyze the semantics of IR
code and generate verification conditions which is further analyzed by
Boogie~\cite{deline2005boogiepl}. We emphasize that both SMACK and \toolname{} uses the DSA
points-to analysis. Hence, neither of them take any advantange of the points-to
analysis, which ususally makes a big impact on the precision of C code
analysis.


We note that as a verification tool, Ct-Verif only reports if the source code
satisfies constant-time property or not\footnote{When verification fails,
Ct-Verify does generate some error messages. But it is hard to decypher those
messages and link them to the source code.}, while \toolname{} can also
pinpoint at source code lines that violate the constant-time property. To
alleviate this limitation of Ct-Verif, whenever Ct-Verif fails, we first log
the line of source code that causes the violation (i.e., the existance of code
at and after that line triggers the failure). Then, we replace the errorous
line with some constant time code that preserves information flow and re-ran
Ct-Verif on the revised code. For example, in
Figure~\ref{code:libgcrypt_limb_processing}, \texttt{e} is tainted at the first
line and is then used in the branch condition at line 4, which makes this
branch vulnerable. Ct-Verif reports an error on the whole program, which we can
trace back to line 4. We record this line and rewrite it with a condition that
only uses public variables, e.g., \texttt{pub\_branch8}. During this iterative
process, we record all errornous lines reported by Ct-Verif and compare them
with the ones found by \toolname{}.
...
\begin{figure}[h!]
    \begin{lstlisting}
        ...
        e = (e << c) << 1;
        ...
        if (pub_branch8) //<@\textcolor{red} {if (e == 0)} @>
        {
            j += c;
            if ( --i < 0 )
            break;
    ...
    \end{lstlisting}
    \caption{Libgcrypt 1.10.1 - mpi-pow.c lines 609-626}
    \label{code:libgcrypt_limb_processing}
\end{figure}

\danfeng{the following is moved from a later section. Try to merge it with the
previous paragraph} To run CtChecker, we feed the whole program to it.
CtChecker will output the exact lines that are positive. This difference in
operation introduces a problem. \textbf{We exploit both tools on four
cryptography libraries, among them, we feed part of the code that achieves the
same functionality, such as encryption. Positives reported outside of these
functions are deemed as out of scope code.} These positives are due to function
calls. When operating Ct-verif in an aforementioned manner, we have to change
some of the out of scope code\danfeng{I sitll don't fully understand the out of
scope issue}. 

\subsubsection{Limitations of Ct-Verif}\danfeng{I'm not sure how much of the
following is still relevant. Please check and remove irrelevant parts}
...
to infer that the memory accesses are in the range of public values.  
...
Both Ct-Verif and \toolname{} are sound analyses; the differences are on false
negtives. To evaluate the false negtives reported by each tool, we consider
four variants of the cryptographic libraries that we evaluated in
Section~\ref{sec:casestudy}.

\begin{itemize}

	\item \emph{WL/FS/FlS/SRC}: this version corresponds to the source code
		under the header ``WL/FS/FlS/SRC'' in
		Table~\ref{tbl:overall-feature-benchmark-new}. 

	\item \emph{baseline}: this version corresponds to the source code
		under the header ``WL/FS/FlS'' in
		Table~\ref{tbl:overall-feature-benchmark-new} 
		%except for BearSSL, which corresponds to the column ``WL/FS/FlS/SRC''. 
		The reason why we cannot include full source for the analysis
		is that other than BearSSL, Ct-Verif runs out of memory when
		their full source code is being analyzed. 

	\item \emph{excluded-undefined-function} \danfeng{check the result of
		our tool when it handles undefined functions in the same way as
		Ct-Verif does. Then decide if we need this version} was
		constructed from the first, the reasons are twofold. First
		reason is to accommodate the difference of how the tools treat
		excluded source. The bottom line is, in this version, we should
		have no function calls whose source is excluded, which could be
		achieved by either adding all the source or removing all the
		excluded function calls. So, version 1 and version 2 of BearSSL
		are the same since Ct-verif runs without problem on it.  For
		other three libraries, we remove all the function calls without
		source. The second reason is to make the comparison more
		meaningful. Both tools taint a function call without source
		conservatively.  This leads to a great portion of false
		positives. These positives does not reflect how
		\toolname{} performs since it can reduce such positives by simply
		including full source. To this end, ruling out this kind of
		false positives is necessary for a meaningful comparison.

	\item \emph{Ct-Verif-Verified} was created from the second by removing
		all positives that are reported by Ct-verif, resulting in fully
		verified code that is constant time. Any positive reported by
		\toolname on this version is expected to be a false positive. 

	\item \emph{CtChecker-Verified} was created from the second by removing
		all positives that are reported by \toolname{}, resulting in
		fully verified code that is constant time. Any positive
		reported by Ct-Verif on this version is expected to be a false
		positive. 
...
Both Ct-Verif and \toolname{} are sound analyses and we did observe that all
true positives are being reported by both tools. The differences are on false
negtives, which is summerized in Table~\ref{tbl:vsctverif}.

\begin{itemize}
	\item \emph{WL/FS/FlS/SRC}: other than BearSSL, Ct-Verif fails
		out-of-memory error since it is not scalable enough to handle
		the full source code of the majority of crptographic libraries.
		
	\item \emph{baseline}: From the result, one can observe there are small
		differences in the number of positives for \toolname~compared
		to Table~\ref{tbl:overall-feature-benchmark-new}. This is an
		adjustment due to the different ways to operate. Ct-Verif would
		report each line once while \toolname~might report one line
		more than once since there might be multiple vulnerable branch
		conditions in one line\danfeng{double check this. Make results
		in Talbe 1 and 4 consistant}. So for those lines that are
		reported by \toolname~more than once, we only count once in
		this comparison. Also, for the aforementioned "out of scope"
		code, even though we carefully do the changes so that the
		information flow remains the same, there still a chance the
		flow might be changed. For a fair comparison, in version 1,
		those positives out of scope will remain changed for \toolname.


	\item \emph{Ct-Verif-Verified}: \danfeng{add observation from the result}

	\item \emph{CtChecker-Verified} \danfeng{add observation from the result}
...
positives reported by each tool in version 2.

\danfeng{add a discussion on what are the causes of false positives being
reported by ct-verif}
