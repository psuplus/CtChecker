...
the correspoinding starting byte offset and the ending byte offset based on the
...
For any given GEP instruction, the points-to analysis has a node in its graph
which represents the memory target of a pointer. For every graph there is a
defined data layout, which has information about alignment and padding of the
data structures which reside in that graph. LLVM also has a structure type
itself 
\subsubsection{Index to Offset}
