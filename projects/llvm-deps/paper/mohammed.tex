...
are no set bits in $e$. If this is the case, the process just moves on to the
...
the value of the entire limb is known.
\begin{algorithm}
...
values based on the value of the limb. The variable $c$ which contains the
number of leading zeros is used in a branch within this else. The outcome of
this branch changes the amount of shift done to the variable which is used as a
key to extract the correct precomputed value.
...
that the number of iterations of the loop is not constant time. 
