...
to find the size through the normalization code given in Algorithm
\ref{alg:libgcrypt_normalize}
...
\end{lstlisting}
  \caption{Libgcrypt - MPN\_NORMALIZE}
  \label{alg:libgcrypt_normalize}
\end{algorithm}
...
Algorithm \ref{alg:libgcrypt_clz}. In this snippet, the number of leading zeros
...
\end{lstlisting}
  \caption{Libgcrypt - Count Leading Zeros}
  \label{alg:libgcrypt_clz}
\end{algorithm}
...
abbreviated main loop code shown in Algorithm \ref{alg:libgcrypt_mainloop}.
...
are no set bits in the \textit{limb} $e$. If this is the case, the process just moves on to the
...
the value of the entire \textit{limb} is known and this is a high-risk result.
\begin{algorithm}
  \caption{Libgcrypt Modular Exponentiation Processing Loop}
  \label{alg:libgcrypt_mainloop}
\begin{lstlisting}
for(;;){
      if (e == 0)
        {
...
first set bit within a \textit{limb}.  The value of $c$ is computed by looking
at each bit from a \textit{limb} in the exponent, making $c$ high-risk. This
branch is in the main processing loop, so it is classified as a looped high-risk result.


\begin{algorithm}
  \caption{Libgcrypt Modular Exponentiation Processing Else Cont'd}
  \label{alg:libgcrypt_elseprocess}
\begin{lstlisting}
...
$j$ depends on the $c0$, a value derived from the limb of the exponent. This
makes $j$ a high-risk variable, and the for loop branch iterates $j > 0$ meaning
...
as indexing inot the precomp array. The conditional set makes it such that for
each value of k, the array value is stored. Similarly, to mask the size of the
answer stored in \textit{base\_u}, the \textit{base\_u\_size} is computed and set each time, but only
the bit mask allows only the true size to be set.

\begin{algorithm}
  \caption{Libgcrypt - Pre-computed table indexing}
  \label{alg:libgcrypt_precomp}
  \begin{lstlisting}
          for (j += W - c0; j >= 0; j--)
            {
...
This loop is done after the main processing loop. The value of \textit{j} is
...
modulus and exponent and so it is considered sensitive. The rsize values are
also considered sensitive due to the flow between parameters in the
\textit{mul\_mod} calls. Both of the branches from the \textit{MPN\_NORMALIZE}
